...
with tab_kpi:
  if selected_kpi == 'TURNOVER':
    ...
    with c1:
      render_metric_card('🎯 Target', format_currency_vn(team_tgt))
    with c2:
      render_metric_card('📈 MTD', format_currency_vn(tot_mtd))
    with c3:
      render_metric_card('📊 % MTD', f'{tot_pct}%')
    with c4:
      render_metric_card('📅 Ngày', format_currency_vn(tot_day))
    ...
    for col in ['Chỉ Tiêu Doanh Số', 'Thực Hiện Ngày', 'Doanh Số MTD']:
      if col in df_display.columns:
        df_display[col] = df_display[col].apply(format_currency_vn)
    ...
    st.info('Vui lòng chọn các báo cáo tương ứng trên bảng điều khiển.')
...
with tab_mcp:
  ...
  if mcp.empty:
    ...
    # Format doanh số hiển thị
    df_f_display = df_f.copy()
    if col_ds and col_ds in df_f_display.columns:
      df_f_display[col_ds] = df_f_display[col_ds].apply(format_currency_vn)

    all_cols = list(df_f_display.columns)
    ...
    if selected_mcp_cols:
      df_f_display = df_f_display[selected_mcp_cols]

    st.dataframe(
        df_f_display, use_container_width=True, height=450, hide_index=True
    )
    st.caption(f'Hiển thị: {len(df_f_display):,} / {len(mcp):,} cửa hàng')
...
with tab_cat:
  ...
  if df_cat.empty:
    ...
    # Format các cột doanh số sang định dạng cách khoảng nghìn
    df_cat_display = df_cat_f.copy()
    for col in df_cat_display.columns:
      if any(
          kw in col.lower()
          for kw in ['doanh số', 'doanh so', 'giá trị', 'tiền']
      ):
        df_cat_display[col] = df_cat_display[col].apply(format_currency_vn)

    all_cat_cols = list(df_cat_display.columns)
    with st.expander('👁️ Chọn cột hiển thị (Category)'):
      selected_cat_cols = st.multiselect(
          'Bỏ chọn để ẩn cột:', all_cat_cols, default=all_cat_cols
      )
    if selected_cat_cols:
      df_cat_display = df_cat_display[selected_cat_cols]

    st.dataframe(
        df_cat_display, use_container_width=True, height=450, hide_index=True
    )
    st.caption(
        f'Hiển thị: {len(df_cat_display):,} / {len(df_cat):,} dòng dữ liệu'
    )
...
with tab_brand:
  ...
  if df_brand.empty:
    ...
    df_brand_display = df_brand_f.copy()
    for col in df_brand_display.columns:
      if any(
          kw in col.lower()
          for kw in ['doanh số', 'doanh so', 'giá trị', 'tiền']
      ):
        df_brand_display[col] = df_brand_display[col].apply(format_currency_vn)

    all_brand_cols = list(df_brand_display.columns)
    ...
    if selected_brand_cols:
      df_brand_display = df_brand_display[selected_brand_cols]

    st.dataframe(
        df_brand_display, use_container_width=True, height=450, hide_index=True
    )
    st.caption(
        f'Hiển thị: {len(df_brand_display):,} / {len(df_brand):,} dòng dữ liệu'
    )
...
with tab_dskh_off:
  ...
  if df_combo_off.empty:
    ...
    df_off_display = df_off_f.copy()
    for col in df_off_display.columns:
      if any(
          kw in col.lower()
          for kw in ['doanh số', 'doanh so', 'giá trị', 'tiền']
      ):
        df_off_display[col] = df_off_display[col].apply(format_currency_vn)

    all_cols_off = list(df_off_display.columns)
    ...
    if selected_cols_off:
      df_off_display = df_off_display[selected_cols_off]

    st.dataframe(
        df_off_display, use_container_width=True, height=450, hide_index=True
    )
    st.caption(
        f'Hiển thị: {len(df_off_display):,} / {len(df_combo_off):,} cửa hàng'
    )
...
with tab_dskh_on:
  ...
  if df_combo_on.empty:
    ...
    df_on_display = df_on_f.copy()
    for col in df_on_display.columns:
      if any(
          kw in col.lower()
          for kw in ['doanh số', 'doanh so', 'giá trị', 'tiền']
      ):
        df_on_display[col] = df_on_display[col].apply(format_currency_vn)

    all_cols_on = list(df_on_display.columns)
    ...
    if selected_cols_on:
      df_on_display = df_on_display[selected_cols_on]

    st.dataframe(
        df_on_display, use_container_width=True, height=450, hide_index=True
    )
    st.caption(
        f'Hiển thị: {len(df_on_display):,} / {len(df_combo_on):,} cửa hàng'
    )
