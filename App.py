...
with head_col2:
    ...
    if not st.session_state['admin_logged_in']:
        ...
        with st.popover("📁 UPLOAD DATA (ADMIN)", use_container_width=True):
            ...
            u_sales = st.file_uploader("📂 Tải lên File Bán Hàng (Data.xlsx / DanhSachChiTietDonHang.xlsx)", type=["xlsx", "csv"], key="u_sales")
            if u_sales: 
...
if file_sales is not None:
    try:
        ...
        for idx, row in raw_peek.iterrows():
            ...
            if any(k in row_str for k in ['sản phẩm', 'product', 'sku', 'item', 'tên sp', 'mã ch', 'outlet code', 'ship-to npp']):
                real_header_row = idx
                break
...
if file_mcp is not None:
    try:
        ...
        # Map Doanh Số MTD bằng cách ép kiểu chuỗi Outlet code chuẩn 100%
