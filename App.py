...
<style>
    ...
    /* ===== CHẶN BÀN PHÍM MOBILE KHI CLICK Ô LỌC ===== */
...
def process_brand_sales(df_rpt, df_brand):
    ...
    brands_list = ["B'fast", "Bupnon TEA365", "Compact", "Chanté", "Chinsu", "Chinsu Story",
...
with tab_mcp:
    ...
    if mcp.empty:
        ...
        with c2:
            ...
            f_thu = st.multiselect("", ["2","3","4","5","6","7","25","36","47"], key="mcp_thu", label_visibility="collapsed")
...
with tab_cat:
    ...
    if df_cat.empty:
        ...
        with c1:
            ...
            f_nv = st.multiselect("", nv_opts, key="cat_nv", label_visibility="collapsed")
        with c2:
            st.markdown('<p class="filter-label">📅 Lọc Theo Thứ</p>', unsafe_allow_html=True)
            f_thu = st.multiselect("", ["2","3","4","5","6","7","25","36","47"], key="cat_thu", label_visibility="collapsed")
...
with tab_brand:
    ...
    if df_brand.empty:
        ...
        with c1:
            ...
            f_nv = st.multiselect("", nv_opts, key="brand_nv", label_visibility="collapsed")
        with c2:
            st.markdown('<p class="filter-label">📅 Lọc Theo Thứ</p>', unsafe_allow_html=True)
            f_thu = st.multiselect("", ["2","3","4","5","6","7","25","36","47"], key="brand_thu", label_visibility="collapsed")
...
with tab_dskh_off:
    ...
    if df_combo_off.empty:
        ...
        with c1:
            ...
            f_off_nv = st.multiselect("", nv_opts_off, key="off_nv", label_visibility="collapsed")
        with c2:
            st.markdown('<p class="filter-label">📅 Lọc Theo Thứ</p>', unsafe_allow_html=True)
            f_off_thu = st.multiselect("", ["2","3","4","5","6","7","25","36","47"], key="off_thu", label_visibility="collapsed")
            
        c3, c4 = st.columns(2)
        with c3:
            st.markdown('<p class="filter-label">🆔 Lọc Mã Khách Hàng</p>', unsafe_allow_html=True)
            f_off_ma = st.text_input("", key="off_ma", label_visibility="collapsed")
        with c4:
            st.markdown('<p class="filter-label">🏪 Lọc Tên Khách Hàng</p>', unsafe_allow_html=True)
            f_off_ten = st.text_input("", key="off_ten", label_visibility="collapsed")
...
with tab_dskh_on:
    ...
    if df_combo_on.empty:
        ...
        with c1:
            ...
            f_on_nv = st.multiselect("", nv_opts_on, key="on_nv", label_visibility="collapsed")
        with c2:
            st.markdown('<p class="filter-label">📅 Lọc Theo Thứ</p>', unsafe_allow_html=True)
            f_on_thu = st.multiselect("", ["2","3","4","5","6","7","25","36","47"], key="on_thu", label_visibility="collapsed")
            
        c3, c4 = st.columns(2)
        with c3:
            st.markdown('<p class="filter-label">🆔 Lọc Mã Khách Hàng</p>', unsafe_allow_html=True)
            f_on_ma = st.text_input("", key="on_ma", label_visibility="collapsed")
        with c4:
            st.markdown('<p class="filter-label">🏪 Lọc Tên Khách Hàng</p>', unsafe_allow_html=True)
            f_on_ten = st.text_input("", key="on_ten", label_visibility="collapsed")
