...
def build_report(df, report_date, targets, report_type, filter_nv=None):
    ...
    nv_label = 'SS Trương Thanh Tân Total' if filter_nv == "Tất cả ĐDKD" else filter_nv
    
    total_row = pd.Dast.markdown("""
<style>
    .main-header {
        background: linear-gradient(90deg, #1a365d 0%, #2b6cb0 100%);
        color: white;
...
def build_report(df, report_date, targets, report_type, filter_nv=None):
    ...
    total_row = pd.DataFrame([{'STT':'-', 'Mã NVBH':'TỔNG CỘNG',
        'Tên NVBH': 'SS Trương Thanh Tân Total' ist.markdown("""
<style>
    .main-header {
        background: linear-gradient(90deg, #1a365d 0%, #2b6cb0 100%);
        color: white;
...
def build_report(df, report_date, targets, report_type, filter_nv=None):
    ...
    total_row = pd.DataFrame([{'STT':'-', 'Mã NVBH':'TỔNG CỘNG',
        'Tên NVBH':'SS Trương Thanh Tân Total' ifst.markdown("""
<style>
    .main-header {
        background: linear-gradient(90deg, #1a365d 0%, #2b6cb0 100%);
        color: white;
...
with tab_kpi:
    if selected_kpi != "COMBO":
        ...
        bottom3 = df_sorted_mtd.tail(3).iloc[::-1]
