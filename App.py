...
COMBO_OFF_PATH = os.path.join(DATA_DIR, combo_off_files[0]) if combo_off_files else "Tân_Combo Kênh OFF.xlsx"
COMBO_ON_PATH  = os.path.join(DATA_DIR, combo_on_files[0]) if combo_on_files else "Tân_Combo Kênh On.xlsx"
...
def filter_by_thu_multi(df, col_thu, f_thu_list):
    if not f_thu_list or not col_thu or df is None or df.empty:
        return df
...
def process_brand_sales(df_rpt, df_brand):
    ...
    c_brand_col = [c for c in df_brand.columns if 'brand' in str(c).lower() and 'danh sách' in str(c).lower()]
    ...
    c_code = [c for c in df_out.columns if 'outlet code' in str(c).lower() or 'mã ch' in str(c).lower()][0]
    c_brand = brand_col_name
    col_val1 = [c for c in df_out.columns if 'doanh số thực đạt của brand' in str(c).lower() and 'not' not in str(c).lower()][0]
    col_val2 = [c for c in df_out.columns if 'doanh số thực đạt của brand' in str(c).lower() and 'not' in str(c).lower()][0]
...
def build_summary_report(df, report_date, df_combo_off_raw, df_combo_on_raw, cat_df, brand_df, mcp_df, filter_nv=None, f_thu_list=None):
    ...
    if not mcp_filtered.empty and f_thu_list:
        c_thu_mcp = find_col(mcp_filtered, ['Thứ', 'Frequency', 'Tần suất', 'thu'])
    ...
    if not off_filtered.empty and f_thu_list:
        c_thu_off = find_col(off_filtered, ['Thứ', 'Frequency', 'thu'])
    ...
    if not on_filtered.empty and f_thu_list:
        c_thu_on = find_col(on_filtered, ['Thứ', 'Frequency', 'thu'])
...
nv_list = sorted(df['Tên NVBH'].dropna().unique().tolist()) if not df.empty and 'Tên NVBH' in df.columns else []
