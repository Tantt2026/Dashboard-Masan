...
with tab_mcp:
    ...
    if mcp.empty:
        ...
        with c2:
            st.markdown(
                '<p class="filter-label">📅 Lọc Theo Thứ</p>',
                unsafe_allow_html=True,
            )
            thu_opts = ['2', '3', '4', '5', '6', '7', '25', '36', '47']
        ...
        with c3:
            st.markdown(
                '<p class="filter-label">🔍 Tìm kiếm (Mã / Tên CH)</p>',
                unsafe_allow_html=True,
            )
            search_key = st.text_input(
                '',
                key='mcp_search_filter',
                placeholder='Nhập mã hoặc tên CH...',
                label_visibility='collapsed',
            )
        ...
        if search_key:
            s_lower = search_key.lower()
            mask_s = pd.Series(False, index=df_f.index)
            if col_ma:
                mask_s = mask_s | df_f[col_ma].astype(str).str.lower().str.contains(s_lower, na=False)
            if col_ten:
                mask_s = mask_s | df_f[col_ten].astype(str).str.lower().str.contains(s_lower, na=False)
            df_f = df_f[mask_s]
...
with tab_cat:
    ...
    if df_cat.empty:
        ...
        c_cat_col = find_col(df_cat, ['Danh sách full cat', 'Category', 'Cat'])
        ...
        with c2:
            st.markdown(
                '<p class="filter-label">📦 Lọc Ngành Hàng (Category)</p>',
                unsafe_allow_html=True,
            )
            cat_opts = (
                sorted(df_cat[c_cat_col].dropna().astype(str).unique().tolist())
                if c_cat_col
                else []
            )
            f_cat = st.multiselect(
                '', cat_opts, key='cat_sub_filter', label_visibility='collapsed'
            )

        df_cat_f = df_cat.copy()
        if f_nv_cat and c_nv_cat:
            df_cat_f = df_cat_f[df_cat_f[c_nv_cat].astype(str).isin(f_nv_cat)]
        if f_cat and c_cat_col:
            df_cat_f = df_cat_f[df_cat_f[c_cat_col].astype(str).isin(f_cat)]
...
with tab_brand:
    ...
    if df_brand.empty:
        ...
        c_brand_name = c_brand_col[0] if c_brand_col else None
        ...
        with c2:
            st.markdown(
                '<p class="filter-label">🏷️ Lọc Thương Hiệu (Brand)</p>',
                unsafe_allow_html=True,
            )
            brand_opts = (
                sorted(df_brand[c_brand_name].dropna().astype(str).unique().tolist())
                if c_brand_name
                else []
            )
            f_brand = st.multiselect(
                '', brand_opts, key='brand_sub_filter', label_visibility='collapsed'
            )
        ...
        if f_brand and c_brand_name:
            df_brand_f = df_brand_f[df_brand_f[c_brand_name].astype(str).isin(f_brand)]
...
with tab_dskh_off:
    ...
    if df_combo_off.empty:
        ...
        c_nv_off_raw = find_col(df_combo_off, ['Tên NV', 'SM name', 'Nhân viên'])
        if c_nv_off_raw:
            st.markdown(
                '<p class="filter-label">👤 Lọc Nhân Viên (ĐDKD)</p>',
                unsafe_allow_html=True,
            )
            off_nv_opts = sorted(df_combo_off[c_nv_off_raw].dropna().astype(str).unique().tolist())
            f_off_nv = st.multiselect('', off_nv_opts, key='off_nv_filter', label_visibility='collapsed')
            
            df_off_f = df_combo_off.copy()
            if f_off_nv:
                df_off_f = df_off_f[df_off_f[c_nv_off_raw].astype(str).isin(f_off_nv)]
        else:
            df_off_f = df_combo_off
...
with tab_dskh_on:
    ...
    if df_combo_on.empty:
        ...
        c_nv_on_raw = find_col(df_combo_on, ['Tên NV', 'SM name', 'Nhân viên'])
        if c_nv_on_raw:
            st.markdown(
                '<p class="filter-label">👤 Lọc Nhân Viên (ĐDKD)</p>',
                unsafe_allow_html=True,
            )
            on_nv_opts = sorted(df_combo_on[c_nv_on_raw].dropna().astype(str).unique().tolist())
            f_on_nv = st.multiselect('', on_nv_opts, key='on_nv_filter', label_visibility='collapsed')
            
            df_on_f = df_combo_on.copy()
            if f_on_nv:
                df_on_f = df_on_f[df_on_f[c_nv_on_raw].astype(str).isin(f_on_nv)]
        else:
            df_on_f = df_combo_on
